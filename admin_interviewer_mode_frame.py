...
class AdminInterviewerModeFrame(BaseSidebarFrame):
    def __init__(self, parent, controller):
        ...
        # --- Sidebar ---
        self.admin_btn = self.add_sidebar_button("ADMIN MODE", self.switch_to_admin_mode)
        self.dash_btn = self.add_sidebar_button("DASHBOARD", self.update_dashboard)
    ...
    def update_dashboard(self):
        ...
        self.tree = ttk.Treeview(self.main_frame, columns=columns, show="headings", height=6)
    ...
    def get_total_interviews(self):
        ...
        try:
            ...
            cursor.execute("SELECT COUNT(*) FROM interviews WHERE user_id = %s", (uid,))
    ...
    def populate_recent_activity(self, search_term="", project_filter=""):
        ...
        try:
            ...
            for rec in records:
                ...
                tl = self.extract_field(summary, "Timeline").replace("→", " to ").strip()
                self.tree.insert("", tk.END, values=(rec[0], ep, proj, tl, rec[2]))
    ...
    def view_all_interviews(self):
        ...
        ctk.CTkButton(self.main_frame, text="← Back to Dashboard", fg_color="gray", command=self.update_dashboard).pack(
            anchor="sw", padx=20, pady=10)
